...
class SaleInReport(models.Model):
    ...
    def init(self):
        ...
        CASE 
            WHEN rp.id in (73961,61248,32471) THEN 286273
            ELSE rp.id 
        END AS partner_id,
        -- rp.id AS partner_id,
        ru.id AS user_id,
        ...
        LEFT JOIN crm_team AS ct ON ct.id = pos.crm_team_id
        ...
        GROUP BY pol.id, pos.id,  pol.name, prod.id, seg.id, rp.id, ru.id, temp.categ_id, u.id, ps.config_id, ct.id, pos.date_order
        -- ORDER BY seg.id, pos.crm_team_id DESC
        -- ORDER BY industry_id, team_id DESC
        ...
        CASE
          ...
          END AS average_price,
        air.invoice_date AS order_date
        ...
        LEFT JOIN res_partner_industry AS seg ON seg.id = rp.industry_id
        GROUP BY air.id, air.id, air.name, prod.id, seg.id, rp.id, air.invoice_user_id, temp.categ_id, u.id, po.id, air.team_id, air.invoice_date
        -- ORDER BY industry_id, team_id ASC
        '''
