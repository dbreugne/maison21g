...
class analytic_report(models.AbstractModel):
    ...
    def _get_lines(self, options, line_id=None):
        res = super(analytic_report, self)._get_lines(options, line_id)
        if options.get('country'):
            ...
            if options.get('country'):
                country_ids = [int(id) for id in options['country']]
                analytic_account_domain += [('partner_id.country_id', 'in', country_ids)]
