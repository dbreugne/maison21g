...
class AccountReport(models.AbstractModel):
    ...
    #Profit and Loss, Balance Sheet, Executive Summary
    @api.model
    def _get_options(self, previous_options=None):
        options = super(AccountReport,self)._get_options(previous_options)
        options['country'] = previous_options and previous_options.get('country')
        return options

    @api.model
    def _init_filter_country(self, options, previous_options=None):
        if previous_options and options.get('country'):
            options['country'] = previous_options['country']
        options['country'] = previous_options and previous_options.get('country') or []
        country_ids = [int(country) for country in options['country']]
        selected_country = country_ids \
                                 and self.env['res.country'].browse(country_ids) \
                                 or self.env['res.country']
        options['selected_country_names'] = selected_country.mapped('name')


    @api.model
    def _get_options_country_domain(self, options):
        domain = []
        if options.get('country'):
            country_ids = [int(acc) for acc in options['country']]
            domain.append(('partner_id.country_id', 'in', country_ids))
        return domain
...
class AccountTrailBalanceReport(models.AbstractModel):
    ...
    @api.model
    def _get_options_domain(self, options):
        domain = super(AccountTrailBalanceReport, self)._get_options_domain(options)
        if options.get('country'):
            domain += [
                ('partner_id.country_id', 'in', options.get('country')),
            ]
...
class AccountCashFlowReport(models.AbstractModel):
    ...
    @api.model
    def _get_options_domain(self, options):
        domain = super(AccountCashFlowReport, self)._get_options_domain(options)
        if options.get('country'):
            domain += [
                ('partner_id.country_id', 'in', options.get('country')),
            ]
...
class AccountTaxReport(models.AbstractModel):
    ...
    @api.model
    def _get_options_domain(self, options):
        domain = super(AccountTaxReport, self)._get_options_domain(options)
        if options.get('country'):
            domain += [
                ('partner_id.country_id', 'in', options.get('country')),
            ]
...
class AccountProfitAndLossReport(models.AbstractModel):
    _inherit = "account.financial.html.report"

    country_ids = fields.Many2many('res.country')

    def _get_filter_info(self, options):
        filter_d , groupby_d = super(AccountProfitAndLossReport,self)._get_filter_info(options)
        if options.get('country'):
            if filter_d:
                filter_d += [
                    ('partner_id.country_id', 'in', options.get('country')),
...
class report_account_aged_partner(models.AbstractModel):
    ...
    #     if options.get('country'):
    #         ctx.update({'country_id': options.get('country')})
    #     res = super(report_account_aged_partner, self)._get_lines(options, line_id)
    #     return res
    ...
    def _get_lines(self, options, line_id=None):
        ...
        if options.get('country'):
            ...
            if options.get('country'):
                context.update({'country_id': options.get('country')})
...
class ReportAgedPartnerBalance(models.AbstractModel):
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move, period_length):
        ...
        res = super(ReportAgedPartnerBalance,self)._get_partner_move_lines(account_type, date_from, target_move, period_length)
        if ctx.get('country_id'):
            periods = {}
            ...
            if ctx.get('country_id'):
                country_clause += 'AND (rc.id IN %s)'
                country_id = self.env['res.country'].search([('id', 'in', ctx.get('country_id'))]).ids
                arg_list += (tuple(country_id or [0]),)
            ...
            query = '''
                ...
                FROM account_move_line AS l
                  ...
                  LEFT JOIN res_country AS rc ON rc.id = res_partner.country_id
