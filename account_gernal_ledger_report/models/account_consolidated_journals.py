...
class report_account_consolidated_journal(models.AbstractModel):
    ...
    def _get_lines(self, options, line_id=None):
        res = super(report_account_consolidated_journal, self)._get_lines(options,line_id)
        if options.get('country'):
            ...
            if len(options.get('country')) > 1:
                country_clause = 'AND rc.id in' + str(tuple(options.get('country')))
            elif len(options.get('country')) == 1:
                country_clause = 'AND rc.id =' + str(options.get('country')[0])
            else:
                country_clause = ''
            ...
            select = """
                ...
                LEFT JOIN res_country AS rc ON rc.id = partner.country_id
                , account_journal j, account_account account, res_company c
                WHERE %s
